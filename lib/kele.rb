...
class Kele
  ...
  def create_message(sender_email, recipient_id, subject)
    response = self.class.post("/messages/", {headers: { "authorization" => token }, body: {sender: sender_email, recipient_id: recipient_id, "stripped-text" => subject }})
    JSON.parse(response.body)
  end

  def create_submission(assignment_branch, assignment_commit_link, comment, enrollment_id)
    # 2162
    response = self.class.post("/checkpoint_submissions/", headers: { "authorization" => token }, body: { checkpoint_id: checkpoint_id, assignment_branch: assignment_branch, assignment_commit_link: assignment_commit_link, comment: comment, enrollment_id: 27555 })
    JSON.parse(response.body)
  end

  def update_submission(submission_id)
    response = self.class.post("/", headers: { "authorization" => token}), body:{})
end
